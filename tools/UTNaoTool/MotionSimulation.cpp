...
MotionSimulation::MotionSimulation(Memory* mem){
...
Memory* MotionSimulation::getMemory(){
  return memory_;
}

void MotionSimulation::getLuaParameters() {
  // get lua parameters for kick
  VisionCore vision_core_(CORE_TOOL_NO_VISION,false,0,1);
  vision_core_.lua_->startLua();
  vision_core_.processVisionFrame();
  KickParamBlock* luaKickParams = NULL;
  vision_core_.memory_->getBlockByName(luaKickParams,"vision_kick_params");
  *kick_params_ = *luaKickParams;
...
void MotionSimulation::updateInputs(){
  ...
  TiltRoll tr = ForwardKinematics::calculateTiltRollFromLeg(true, joint_angles_->values_, dimensions_);
