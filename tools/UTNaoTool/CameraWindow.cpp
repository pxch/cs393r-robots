...
void CameraWindow::update(Memory* memory) {
...
void CameraWindow::drawRawImage(ImageWidget *image) {
  ...
  if (image_block_ == NULL || image_block_->img_top_ == NULL || image_block_->img_bottom_ == NULL) {
    image->fill(0);
    return;
  }
  ...
  ImageParams* iparams;
  if (showTop) {
    imgPtr = image_block_->img_top_.get();
    iparams = &image_block_->top_params_;
  }
  else {
    imgPtr = image_block_->img_bottom_.get();
...
void CameraWindow::sendCommand(QString cmd) {
  ((UTMainWnd*)parent_)->sendUDPCommand(((UTMainWnd*)parent_)->getCurrentAddress(),cmd);
}
