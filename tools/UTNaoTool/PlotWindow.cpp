...
#ifdef ON_LAB_MACHINE
  plot_.setCanvasBackground(QBrush("white"));
#else
  plot_.setCanvasBackground("white");
#endif
...
#else
  plot_curves_[index]->setData(&times_[0],data,memory_log_->size());
#endif
...
void PlotWindow::setMemoryLog(std::vector<Memory> *memory_log) {
  memory_log_ = memory_log;

  for (unsigned int i = 0; i < memory_log_->size(); i++)
    populateGraphableBlock((*memory_log_)[i]);
