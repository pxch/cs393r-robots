...
void ClassificationWidget::generateColorTable() {
    ...
    for(int frame = 0; frame < maxFrames_; frame++) {
        unsigned char* image = images[frame];
        int count = annotations_.size();
        for(int i = 0; i < count; i++){
            Annotation* annotation = annotations_[i];
...
void ClassificationWidget::handleNewLogLoaded(Log* log){
    log_ = log;
    maxFrames_ = log_->size();
}

void ClassificationWidget::setAnnotations(std::vector<Annotation*> annotations){
