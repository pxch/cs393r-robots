...
void RobotGL::drawStickFigure(Vector3<float> *pos,RGB body_color, RGB left_color, RGB right_color) {
  ...
  // Draw joint markers
  for (int i=0; i<BodyFrame::NUM_POINTS-8; i++) { // Don't draw spheres for the feet
    //basicGL.drawSphere(pos[i].translation,20);
  ...
  basicGL.drawLine(pos[BodyFrame::left_shoulder],pos[BodyFrame::right_shoulder]);
  Vector3<float> mid_shoulder = (pos[BodyFrame::left_shoulder]+pos[BodyFrame::right_shoulder])/2.0;
  basicGL.drawLine(pos[BodyFrame::head],mid_shoulder);

  basicGL.drawLine(pos[BodyFrame::left_shoulder],pos[BodyFrame::left_elbow]);
  basicGL.drawLine(pos[BodyFrame::left_elbow],pos[BodyFrame::left_wrist]);

  basicGL.drawLine(pos[BodyFrame::right_shoulder],pos[BodyFrame::right_elbow]);
  basicGL.drawLine(pos[BodyFrame::right_elbow],pos[BodyFrame::right_wrist]);


  basicGL.drawLine(pos[BodyFrame::left_hip],pos[BodyFrame::right_hip]);
  Vector3<float> mid_hip = (pos[BodyFrame::left_hip]+pos[BodyFrame::right_hip])/2.0;
  basicGL.drawLine(mid_shoulder,mid_hip);

  basicGL.drawLine(pos[BodyFrame::left_hip],pos[BodyFrame::left_knee]);
  basicGL.drawLine(pos[BodyFrame::left_knee],pos[BodyFrame::left_ankle]);
  basicGL.drawLine(pos[BodyFrame::left_ankle],pos[BodyFrame::left_foot]);

  basicGL.drawLine(pos[BodyFrame::right_hip],pos[BodyFrame::right_knee]);
  basicGL.drawLine(pos[BodyFrame::right_knee],pos[BodyFrame::right_ankle]);
  basicGL.drawLine(pos[BodyFrame::right_ankle],pos[BodyFrame::right_foot]);
  ...
  // Draw feet
  drawFoot(left_color,pos[BodyFrame::left_foot_front_left],pos[BodyFrame::left_foot_front_right],pos[BodyFrame::left_foot_rear_right],pos[BodyFrame::left_foot_rear_left]);

  drawFoot(right_color,pos[BodyFrame::right_foot_front_left],pos[BodyFrame::right_foot_front_right],pos[BodyFrame::right_foot_rear_right],pos[BodyFrame::right_foot_rear_left]);
...
void RobotGL::drawFoot(RGB color, Vector3<float> pt1, Vector3<float> pt2, Vector3<float> pt3, Vector3<float> pt4){
  ...
  basicGL.drawRectangle(pt1,pt2,pt3,pt4);
}
...
void RobotGL::drawGroundCoM(Vector3<float> CoM) {
  basicGL.colorRGBAlpha(basicGL.blueRGB,0.75);
  basicGL.drawSphere(CoM, 10);
}

void RobotGL::drawFootCommand(Vector3<float> command) {
  basicGL.colorRGBAlpha(basicGL.yellowRGB,0.75);
...
void RobotGL::drawSimpleRobot() {
  ...
  // body cylinder
  basicGL.drawCylinder(30, 250);

  // head
  Vector3<float> headHeight;
  headHeight.z = 250.0;
  basicGL.drawSphere(headHeight, 54.0f);
