...
ExtrinsicCalibrationWidget::ExtrinsicCalibrationWidget(QWidget* parent) : QWidget(parent) {
  ...
  connect(optimizeButton, SIGNAL(clicked()), this, SLOT(optimizeCalibration()));
  ...
  connect(topTilt,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(topRoll,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(topYaw,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(bottomTilt,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(bottomRoll,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(bottomYaw,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(poseX,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(poseY,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(poseTheta,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  ...
  connect(topFOVx,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(topFOVy,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(bottomFOVx,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
  connect(bottomFOVy,SIGNAL(valueChanged(double)), this, SIGNAL(calibrationsUpdated()));
...
RobotCalibration ExtrinsicCalibrationWidget::getCalibration() const {
  RobotCalibration cal;
...
void ExtrinsicCalibrationWidget::optimizeCalibration() {
  //RobotCalibration cal = getCalibration();
  //int iters = iterations->value();
  //TODO: pass in the top or bottom image processor based on calibration selections
  //ExtrinsicCalibrator calibrator(world_object_block_, processor);
  //calibrator.calibrate(cal, samples_, iters);
  //loadCalibration(cal);
  //float e = calibrator.getSampleErrorCameraFrame(samples_, cal);
  //error->setValue(e);
}
