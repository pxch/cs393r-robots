...
void MotionWidget::draw() {
  ...
  if (displayOptions[SHOWBODYMODEL]) drawBodyModel(body_model_, basicGL_.whiteRGB);
  ...
  if (walk_engine_ != NULL){
    ...
    // show ref trajectory
    if (displayOptions[SHOWZMPREF]) drawZmpTrajectory(basicGL_.greenRGB, walk_engine_->zmp_ref_);
    // show current
    if (displayOptions[SHOWCURRENTZMP]) drawZmp(basicGL_.orangeRGB, walk_engine_->current_state_.zmp_, 0);
    // show desired next
    if (displayOptions[SHOWDESIREDZMP]) drawZmp(basicGL_.pinkRGB, walk_engine_->desired_next_state_.zmp_, timeInc);
    // show sensed
    if (displayOptions[SHOWSENSEDZMP]) drawZmp(basicGL_.yellowRGB, walk_engine_->sensor_zmp_, 0);

    // show state com
    if (displayOptions[SHOWCURRENTPEN]) drawPen(basicGL_.purpleRGB, walk_engine_->current_state_.pen_pos_, 0);
    // show desired next com
    if (displayOptions[SHOWDESIREDPEN]) drawPen(basicGL_.grayRGB, walk_engine_->desired_next_state_.pen_pos_, timeInc);
  }
...
void MotionWidget::drawSwingFoot(){
  ...
  drawFoot(swing_foot_,basicGL_.pinkRGB);
}
...
void MotionWidget::drawPen(RGB color, Vector2<float> val, float time){
  ...
  if (walk_param_ != NULL) {
    drawingPen.translation.z = walk_param_->params_.walk_height_;
    // plot this point
    basicGL_.colorRGBAlpha(color, 0.7);
    basicGL_.drawSphere(drawingPen.translation, 8);
  }
...
void MotionWidget::drawStep(WalkEngineBlock::Step s, bool draw_on_ground){
  ...
  RGB color = basicGL_.redRGB;
  if (s.is_left_foot_){
    color = basicGL_.blueRGB;
  }
  if (s.is_stand_)
    color = basicGL_.greenRGB;
...
void MotionWidget::drawTargetPoint(){
  ...
  basicGL_.colorRGBAlpha(basicGL_.pinkRGB, 0.7);
...
BodyModelBlock* MotionWidget::getBodyModelFromJoints(float *joints){
  ...
  ForwardKinematics::calculateRelativePose(joints, sensors_->values_[angleX], sensors_->values_[angleY], new_body_model_->rel_parts_, dimensions_.values_);
  Pose3D base = ForwardKinematics::calculateVirtualBase(sensors_->values_, new_body_model_->rel_parts_);
  //ForwardKinematics::calculateRelativePose(joints, 0, 0, new_body_model_->rel_parts_, dimensions_);
...
void MotionWidget::drawKickFootTarget(){
  if (motion_sim_->use_com_kick_) {
    if (kick_module_ == NULL || body_model_ == NULL || joint_values_ == NULL) return;
  } else{
    if (kick_engine_ == NULL || body_model_ == NULL || joint_values_ == NULL) return;
  }
  ...
  Pose3D torso = new_body_model_->abs_parts_[BodyPart::torso];
  ...
  drawFoot(curr_foot,basicGL_.blueRGB);
  drawFoot(target_foot,basicGL_.redRGB);
}
...
void MotionWidget::drawKickSpline(bool names){
  ...
  if (!names) {
    basicGL_.colorRGBAlpha(basicGL_.blueRGB, 0.9);
  ...
  if (kick_stance_spline_.isInitialized()) {
    for (int i = 0; i < kick_params_->params_.num_stance_spline_pts; i++) {
      ...
      stance_pos += body_model_->abs_parts_[BodyPart::torso].translation;
      stance_pos += body_model_->abs_parts_[BodyPart::right_bottom_foot].translation - body_model_->abs_parts_[BodyPart::right_ankle].translation;

      // set z as time
      stance_pos.z = body_model_->abs_parts_[BodyPart::right_bottom_foot].translation.z + stance_height * kick_params_->params_.spline_stance_times[i] / kick_stance_spline_.totalTime();

      name = i;
      if (selectedName() == name)
        basicGL_.colorRGBAlpha(basicGL_.redRGB, 0.9);
      else
        basicGL_.colorRGBAlpha(basicGL_.greenRGB, 0.9);
      if (names)
        glPushName(name);
...
void MotionWidget::calcKickSplinePoints() {
  ...
  for (int i = 0; i < num_spline_iterp_points; i++) {
    ...
    if (kick_stance_spline_.isInitialized()) {
      ...
      stance_pos.z = body_model_->abs_parts_[BodyPart::right_bottom_foot].translation.z + stance_height * time / kick_stance_spline_.totalTime();
...
void MotionWidget::calculateStickFigure(BodyModelBlock* bm_) {
  ...
  stick_figure_[BodyFrame::head]=bm_->abs_parts_[BodyPart::head].translation;

  // Left Arm
  stick_figure_[BodyFrame::left_shoulder]=bm_->abs_parts_[BodyPart::left_shoulder].translation;
  stick_figure_[BodyFrame::left_elbow]=bm_->abs_parts_[BodyPart::left_forearm].translation;
  Pose3D t = bm_->abs_parts_[BodyPart::left_forearm];
  stick_figure_[BodyFrame::left_wrist]=t.translate(120,0,0).translation;

  // Right Arm
  stick_figure_[BodyFrame::right_shoulder]=bm_->abs_parts_[BodyPart::right_shoulder].translation;
  stick_figure_[BodyFrame::right_elbow]=bm_->abs_parts_[BodyPart::right_forearm].translation;
  t = bm_->abs_parts_[BodyPart::right_forearm];
  stick_figure_[BodyFrame::right_wrist]=t.translate(120,0,0).translation;

  // Left Leg
  stick_figure_[BodyFrame::left_hip]=bm_->abs_parts_[BodyPart::left_hip].translation;
  stick_figure_[BodyFrame::left_knee]=bm_->abs_parts_[BodyPart::left_tibia].translation;
  t = bm_->abs_parts_[BodyPart::left_tibia];
  stick_figure_[BodyFrame::left_ankle]=t.translate(0,0,-102.75).translation;
  Pose3D foot = bm_->abs_parts_[BodyPart::left_bottom_foot];
  ...
  //Right Leg
  stick_figure_[BodyFrame::right_hip]=bm_->abs_parts_[BodyPart::right_hip].translation;
  stick_figure_[BodyFrame::right_knee]=bm_->abs_parts_[BodyPart::right_tibia].translation;
  t = bm_->abs_parts_[BodyPart::right_tibia];
  stick_figure_[BodyFrame::right_ankle]=t.translate(0,0,-102.75).translation;
  t = bm_->abs_parts_[BodyPart::right_ankle];
  foot = bm_->abs_parts_[BodyPart::right_bottom_foot];
...
void MotionWidget::textZmpCom(){
  ...
  if (body_model_ != NULL){
    ...
    Pose3D stance_foot_;
    stance_foot_ = body_model_->abs_parts_[BodyPart::left_bottom_foot];
    if (walk_engine_->step_current_.is_left_foot_){
      stance_foot_ = body_model_->abs_parts_[BodyPart::left_bottom_foot];
    } else {
      stance_foot_ = body_model_->abs_parts_[BodyPart::right_bottom_foot];
    }
...
void MotionWidget::textWalkRequest(){
  ...
  renderText(x,y+=15,    "Walk Command: "+QString(walkNames[walk_request_->motion_].c_str()));
...
void MotionWidget::drawAbsFeetTargets(){
  ...
  drawFoot(left_foot,basicGL_.yellowRGB);
  drawFoot(right_foot,basicGL_.yellowRGB);
}
...
void MotionWidget::keyPressEvent(QKeyEvent *event) {
  ...
  // kick sim mode
    ...
    case Qt::Key_S:
      if (event->modifiers() & Qt::ControlModifier) {
        std::string filename = std::string(getenv("NAO_HOME")) + "/core/lua/cfgkickspline.lua";
        std::cout << "SAVING Parameters at: " << filename << std::endl;
        kick_params_->params_.saveSplines(filename);
      } else {
