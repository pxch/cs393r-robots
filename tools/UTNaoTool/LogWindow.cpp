...
LogWindow::LogWindow(QMainWindow* p) {
  setupUi(this);
...
// load the text file
void LogWindow::loadTextFile(const char* cName){
  string fileName=cName;
  string fName = fileName.replace(fileName.size()-3, 3, "txt");
  cout << "Opening Text Log  " << fName << flush;

  ifstream textFile(fName.c_str());
  textEntries.clear();
  ...
  char buffer[2048];
  while (textFile.getline(buffer,2048)) {
    textEntries.push_back(string(buffer));
  }
  cout << "  - Done with " << textEntries.size() << " lines!" << "\n";
  updateFrame(NULL);
}

void LogWindow::loadConfig(QTextStream &t) {
  bool ok;
  int num;
  num = UTMainWnd::loadInt(t,ok);
  if (!ok)
    return;
  logLevelLowBox->setValue(num);
  num = UTMainWnd::loadInt(t,ok);
  if (!ok)
    return;
  logLevelHighBox->setValue(num);
  num = UTMainWnd::loadInt(t,ok);
  if (!ok)
    return;
  moduleType = num;
...
void LogWindow::saveConfig(QTextStream &t) {
  t << logLevelLowBox->value() << "\n";
  t << logLevelHighBox->value() << "\n";
  t << moduleType << "\n";
}
...
void LogWindow::updateFrame(Memory* mem) {
  ...
  for (int i = 0; i < (int)textEntries.size(); i++) {
    ...
    if (moduleType != NUM_MODULE_TYPES && moduleType != module)
      continue;
...
void LogWindow::updateMemoryBlocks(Memory *mem) {
  QString text;
