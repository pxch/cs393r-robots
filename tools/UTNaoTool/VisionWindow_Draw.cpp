...
void VisionWindow::redrawImages() {
	...
	redrawImages(rawImageTop, segImageTop, objImageTop, horizontalBlobImageTop,
			verticalBlobImageTop, transformedImageTop);
	redrawImages(rawImageBottom, segImageBottom, objImageBottom,
			horizontalBlobImageBottom, verticalBlobImageBottom,
			transformedImageBottom);
...
void VisionWindow::updateBigImage(ImageWidget* source) {
	...
	int width = processor->getImageWidth(), height =
...
void VisionWindow::redrawImages(ImageWidget* rawImage, ImageWidget* segImage,
		ImageWidget* objImage, ImageWidget* horizontalBlobImage,
		ImageWidget* verticalBlobImage, ImageWidget* transformedImage) {
...
void VisionWindow::drawCalibrationField(ImageWidget *image) {
	...
	for (unsigned int i = 0; i < calSamples.size(); i++) {
		...
		if ((s.camera == Camera::TOP && img == IMAGE_TOP)
				|| (s.camera == Camera::BOTTOM && img == IMAGE_BOTTOM)) {
			painter.setPen(spen);
	...
	for (vector<LineSegment>::iterator i = segments.begin();
			i != segments.end(); ++i) {
		...
		if (segment.start.x > iparams.width && segment.end.x < 0)
			continue;
		painter.drawLine(segment.start.x, segment.start.y, segment.end.x,
				segment.end.y);
		painter.drawEllipse((int) segment.start.x, (int) segment.start.y,
				pointWidth, pointWidth);
		painter.drawEllipse((int) segment.end.x, (int) segment.end.y,
				pointWidth, pointWidth);
	}
}
	...
	if (doingClassification_) {
		...
		for (uint16_t y = 0; y < iparams.height; y++) {
			for (uint16_t x = 0; x < iparams.width; x++) {
				Color c = ColorTableMethods::xy2color(rawImg, colorTable, x, y,
						iparams.width);
...
void VisionWindow::drawLines(ImageWidget *image) {
	...
	for (int i = 0; i < FieldLinesCounter; i++) {
		...
		painter.setPen(wpen);
		int x1, y1, x2, y2;
		getPointsForLine(processor->line_detector_->fieldLines[i],
				processor->line_detector_->fieldLines[i]->isHorizontal, x1, y1,
				x2, y2);

		if (processor->line_detector_->fieldLines[i]->isCircle
				|| processor->line_detector_->fieldLines[i]->isCurve) {
			// draw an actual curve for these
			QPainterPath path;
			path.moveTo(
					processor->line_detector_->fieldLines[i]->PointsArray[0]->PosX,
					processor->line_detector_->fieldLines[i]->PointsArray[0]->PosY);
			for (unsigned int pt = 3;
					pt < processor->line_detector_->fieldLines[i]->Points; pt +=
							3) {
				path.lineTo(
						processor->line_detector_->fieldLines[i]->PointsArray[pt]->PosX,
						processor->line_detector_->fieldLines[i]->PointsArray[pt]->PosY);
			}
...
void VisionWindow::drawYellowGoal(ImageWidget *image) {
	if (IS_RUNNING_CORE) {
		...
		for (int i = 0; i < FieldLinesCounter; i++) {
			...
			int x1, y1, x2, y2;
			getPointsForLine(processor->goal_detector_->yellowPosts[i], false,
					x1, y1, x2, y2);
			painter.drawLine(x1, y1, x2, y2);
			int xTop = -processor->goal_detector_->yellowPosts[i]->Offset
					/ processor->goal_detector_->yellowPosts[i]->Slope;
...
void VisionWindow::getPointsForLine(FieldLine *fieldLine, bool isHorizontal,
...
void VisionWindow::drawBall(ImageWidget* image) {
	...
	if (IS_RUNNING_CORE) {
		...
		int r = best->radius;
		painter.drawEllipse((int) best->centerX - r - 1,
		...
		if ((ball->fromTopCamera && _widgetAssignments[image] == IMAGE_BOTTOM)
				|| (!ball->fromTopCamera
						&& _widgetAssignments[image] == IMAGE_TOP))
			return;
		int radius = ball->radius;
		painter.drawEllipse(ball->imageCenterX - radius,
				ball->imageCenterY - radius, radius * 2, radius * 2);
	}
}
...
void VisionWindow::drawBallCands(ImageWidget* image) {
	...
	for (uint16_t i = 0; i < processor->ball_detector_->candidateCount; i++) {
		...
		painter.drawEllipse((int) candidate->centerX - r,
...
void VisionWindow::drawTransformedPoints(ImageWidget *image) {
	...
	for (int i = 0; i < processor->line_detector_->FieldLinesCounter; i++) {
		for (unsigned int j = 0;
				j < processor->line_detector_->fieldLines[i]->Points; j++) {
			int x =
					-1
							* processor->line_detector_->fieldLines[i]->TranPointsArray[j]->PosY
							* magnification + iparams.width / 2;
			int y =
					iparams.height
							- (2000
									+ processor->line_detector_->fieldLines[i]->TranPointsArray[j]->PosX)
									* magnification;
...
void VisionWindow::drawVertLinePoints(ImageWidget *image) {
	...
	for (int x = 0; x < iparams.width; x += hstep) {
		for (uint16_t i = 0;
				i < processor->classifier_->verticalPointCount[c_WHITE][x];
				i++) {
			VisionPoint *lp =
					&processor->classifier_->verticalPoint[c_WHITE][x][i];
	...
	for (uint16_t i = 0;
			i < processor->blob_detector_->verticalBlob[c_WHITE].size(); i++) {
		Blob *lb = &processor->blob_detector_->verticalBlob[c_WHITE][i];
		VisionPoint *lpi =
				&processor->classifier_->verticalPoint[c_WHITE][lb->lpIndex[0]
						>> 16][lb->lpIndex[0] & 0xfffful];
		VisionPoint *lpf =
				&processor->classifier_->verticalPoint[c_WHITE][lb->lpIndex[lb->lpCount
						- 1] >> 16][lb->lpIndex[lb->lpCount - 1] & 0xfffful];
		painter.drawLine(lpi->xi, (lpi->yi + lpi->yf) / 2, lpf->xf,
				(lpf->yi + lpf->yf) / 2);
	}
	...
	for (int x = 0; x < iparams.width; x += hstep) {
		for (uint16_t i = 0;
				i < processor->classifier_->verticalPointCount[c_BLUE][x];
				i++) {
			VisionPoint *lp =
					&processor->classifier_->verticalPoint[c_BLUE][x][i];
			painter.drawLine(lp->xi, lp->yi, lp->xf, lp->yf);
		}
	}
	...
	for (uint16_t i = 0;
			i < processor->blob_detector_->verticalBlob[c_BLUE].size(); i++) {
		Blob *lb = &processor->blob_detector_->verticalBlob[c_BLUE][i];
		VisionPoint *lpi =
				&processor->classifier_->verticalPoint[c_BLUE][lb->lpIndex[0]
						>> 16][lb->lpIndex[0] & 0xfffful];
		VisionPoint *lpf =
				&processor->classifier_->verticalPoint[c_BLUE][lb->lpIndex[lb->lpCount
						- 1] >> 16][lb->lpIndex[lb->lpCount - 1] & 0xfffful];
		painter.drawLine(lpi->xi, (lpi->yi + lpi->yf) / 2, lpf->xf,
				(lpf->yi + lpf->yf) / 2);
	}
	...
	for (int x = 0; x < iparams.width; x += hstep) {
		for (uint16_t i = 0;
				i < processor->classifier_->verticalPointCount[c_PINK][x];
				i++) {
			VisionPoint *lp =
					&processor->classifier_->verticalPoint[c_PINK][x][i];
			painter.drawLine(lp->xi, lp->yi, lp->xf, lp->yf);
		}
	}
	...
	for (uint16_t i = 0;
			i < processor->blob_detector_->verticalBlob[c_PINK].size(); i++) {
		Blob *lb = &processor->blob_detector_->verticalBlob[c_PINK][i];
		VisionPoint *lpi =
				&processor->classifier_->verticalPoint[c_PINK][lb->lpIndex[0]
						>> 16][lb->lpIndex[0] & 0xfffful];
		VisionPoint *lpf =
				&processor->classifier_->verticalPoint[c_PINK][lb->lpIndex[lb->lpCount
						- 1] >> 16][lb->lpIndex[lb->lpCount - 1] & 0xfffful];
		painter.drawLine(lpi->xi, (lpi->yi + lpi->yf) >> 1, lpf->xf,
				(lpf->yi + lpf->yf) >> 1);
	}
}
...
void VisionWindow::drawHorzLinePoints(ImageWidget *image) {
	...
	painter.setPen(pen);
	for (uint16_t i = 0;
			i < processor->blob_detector_->horizontalBlob[c_BLUE].size();
			i++) {
...
void VisionWindow::drawDetectedRobots(ImageWidget *image) {
	...
	if (IS_RUNNING_CORE) {
		ImageProcessor* processor = getImageProcessor(image);
		std::list<Blob*> pink = processor->robot_detector_->getPinkRobots(),
				blue = processor->robot_detector_->getBlueRobots();
		QPainter painter(image->getImage());
		for (std::list<Blob*>::iterator it = pink.begin(); it != pink.end();
				it++) {
			Blob* blob = *it;
		...
		for (std::list<Blob*>::iterator it = blue.begin(); it != blue.end();
				it++) {
			Blob* blob = *it;
		...
		for (int i = WO_OPPONENT_FIRST; i <= WO_OPPONENT_LAST; i++) {
			...
			if (!wo->seen || (wo->fromTopCamera != (cam == IMAGE_TOP)))
				continue;
			painter.drawEllipse(wo->imageCenterX, wo->imageCenterY, width,
					height);
		}
	}
}
...
void VisionWindow::drawWorldObject(ImageWidget* image, QColor color,
		int worldObjectID) {
	if (world_object_block_ != NULL) {
		...
		if ((object->fromTopCamera && _widgetAssignments[image] == IMAGE_BOTTOM)
				|| (!object->fromTopCamera
						&& _widgetAssignments[image] == IMAGE_TOP))
			return;
		int offset = 5;
		int x1, y1, x2, y2;

		x1 = object->imageCenterX - offset, y1 = object->imageCenterY - offset, x2 =
				object->imageCenterX + offset, y2 = object->imageCenterY
				+ offset;
