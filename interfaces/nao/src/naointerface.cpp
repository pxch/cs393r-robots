 ...
 */
naointerface::naointerface( AL::ALPtr<AL::ALBroker> broker, const std::string& name):
  AL::ALModule(broker, name)
{
  std::cout << "NaoInterface::Starting Interface Module" << std::endl << std::flush;

  fast_sensor_access_ = (AL::ALPtr<AL::ALMemoryFastAccess>(new AL::ALMemoryFastAccess()));
...
void naointerface::start() {
  ...
  try {
    ...
    tts_proxy_ = AL::ALPtr<AL::ALTextToSpeechProxy>(new AL::ALTextToSpeechProxy(getParentBroker()));
...
void naointerface::initMemory() {
  // setup the memory
  memory_ = new Memory(true,MemoryOwner::INTERFACE,0,1,true);
  ...
  // Initialise robot to sitting pose
  //processed_joint_commands_->setPoseDeg(Pose::sitting);
