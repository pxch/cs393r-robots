...
BlobCollection BlobDetector::mergeBlobs(BlobCollection& blobs, int thresholdX, int thresholdY) {
  ...
  uint16_t mergeIndex[blobs.size()];
  mergeBlobs(blobs, (uint16_t*)mergeIndex, blobs.size(), thresholdX, thresholdY);
  BlobCollection merged;
  for (uint16_t i = 0; i < blobs.size(); i++)
    if(mergeIndex[i] == blobs.size())
      merged.push_back(blobs[i]);
  return merged;
}
...
uint16_t BlobDetector::mergeBlobs(BlobCollection& blobs, uint16_t *mergeIndex, int mergeCount, int thresholdX, int thresholdY) {
  // Initialize
  for (uint16_t i = 0; i < blobs.size(); i++)
  ...
  for (uint16_t i = 0; i < blobs.size(); i++) {
    ...
    for (uint16_t j = i + 1; j < blobs.size(); j++) {
      ...
      bool closeX = (abs((int)bli->xf - (int)blj->xi) < thresholdX);
      bool closeY = (abs((int)bli->yf - (int)blj->yi) < thresholdY);
