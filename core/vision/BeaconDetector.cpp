...
//              vertical_dist_thres = 15;
	if (blob1.dy < blob2.dy)
		vertical_dist_thres = blob1.dy >> 2;
	else
		vertical_dist_thres = blob2.dy >> 2;
	...
	if (float(blob1.dx + blob1.dy) / float(blob2.dx + blob2.dy) > 1.3
			|| float(blob1.dx + blob1.dy) / float(blob2.dx + blob2.dy) < 0.7)
		return false;
