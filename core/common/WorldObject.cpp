...
bool WorldObject::isGoalCenter() {
	return (type == WO_OPP_GOAL || type == WO_OWN_GOAL
			|| type == WO_UNKNOWN_GOAL);
}
...
bool WorldObject::isOwnGoal() {
	return (type == WO_OWN_GOAL || type == WO_OWN_RIGHT_GOALPOST
			|| type == WO_OWN_LEFT_GOALPOST);
}

bool WorldObject::isOppGoal() {
	return (type == WO_OPP_GOAL || type == WO_OPP_RIGHT_GOALPOST
			|| type == WO_OPP_LEFT_GOALPOST);
}
...
bool WorldObject::isAmbiguous() {
	return (isUnknownGoal() || isUnknownIntersection() || isUnknownLine()
			|| isUnknownPenaltyCross());
}
